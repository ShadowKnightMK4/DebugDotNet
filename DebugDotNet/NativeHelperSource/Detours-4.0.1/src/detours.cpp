...
LONG WINAPI DetourUpdateThread(_In_ HANDLE hThread)
{
    ...
    */
    if (hThread == GetCurrentThread())
    {
        //OutputDebugString(L"Skipping DetourUpdateThread() as Current thread is the target (hThread == GetCurrentTHread())");
        return NO_ERROR;
    }
    else
    {
        if (hThread != INVALID_HANDLE_VALUE)
        {
            ...
            {
            //    OutputDebugString(L"Skipping DetourUpdateThread() as Current thread is the target    if (GetCurrentThreadId() == GetThreadId(hThread))");
                return NO_ERROR;
            }
            else
            {
              //  OutputDebugStringW(L"Not skipping DetourUpdateThread(), Target and current thread are different");
            }
        }
        else
        {
           // OutputDebugStringW(L"hThread == INVALID_HANDLE_VALUE");
        }
    }
